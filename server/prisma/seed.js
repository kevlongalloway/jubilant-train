/**
 * Database seed — 20 users, 25 books, 200+ posts, interactions, follows.
 * Idempotent: re-seeds if fewer than 18 users exist (wipes first).
...
const REVIEW_TEMPLATES = [
  ...
  'The structure of {title} is its argument. {author} knows this. Every formal choice is thematic. 4.5/5 and I\'m annoyed it\'s over.',
  ...
  'I read {title} too fast the first time. That was my mistake. {author} is writing for readers who are willing to slow down. Second read was slower and completely different.',
...
const RECOMMENDATION_TEMPLATES = [
  ...
  '{title} has been sitting in my recommendations queue for a year. I finally picked it up and now I\'m the person recommending it. {author} earns every page.',
  'For anyone who loved {title}: {author} has this gift for making the interior life feel as dramatic as anything external. We don\'t talk about that enough.',
  'Start {title} on a weekend. You will not want to stop. {author} builds momentum the way few writers can — scene by scene until you\'re in it.',
...
const ORIGINAL_TEMPLATES = [
  ...
  'I read the last page first when I was young. Now I think about what that impulse was — the fear that the story wouldn\'t survive. That nothing would.',
  'The best annotation I ever read was in a library copy. Seventeen words in the margin of a book I\'d been trying to understand for years. I never knew who wrote them.',
  ...
  'A character I loved just died in the book I\'m reading. I didn\'t close it. I sat with it for a while. That\'s what the author wanted. I think they were right.',
...
function randomPast(maxDaysAgo = 7) {
...
async function seed() {
  ...
  for (const data of USERS) {
    const inkBase = [800, 1200, 1600, 2400, 3200, 4800][Math.floor(Math.random() * 6)];
    const user = await prisma.user.create({
      data: { ...data, password, ink: inkBase + Math.floor(Math.random() * 400) },
    });
  ...
  // Reviews — each book gets 3–5 reviews from different users
  ...
  // Recommendations — each book gets 2–3 recommendations
  for (const book of books) {
    const count = 2 + Math.floor(Math.random() * 2);
    const recommenders = [...users].sort(() => Math.random() - 0.5).slice(0, count);
  ...
  // Original writing — each user writes 4–7 original posts
  ...
  for (const post of posts) {
    ...
    for (const user of engagers) {
      // Each engager views, and may also like/save
      const types = ['view'];
      if (Math.random() > 0.35) types.push('like');
      if (Math.random() > 0.65) types.push('save');
        ...
        try {
          await prisma.interaction.create({
            data: {
              ...
              createdAt: new Date(new Date(post.createdAt).getTime() + Math.random() * 3_600_000 * 12),
            },
          });
          interactionCount++;
  ...
  console.log(`  All demo accounts use password: precis123`);
}
